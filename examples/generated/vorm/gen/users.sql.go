...
type PaginateUsersCursorParams struct {
	Cursor  string
	PerPage int
}
...
// PaginateUsersCursor is generated from // vorm:query in users.go
func PaginateUsersCursor(ctx context.Context, db query.DB, arg PaginateUsersCursorParams) (*query.PageResult[PaginateUsersCursorRow], error) {
	perPage := arg.PerPage
	if perPage <= 0 {
		perPage = 15
	}
	...
	if cursor != "" {
		var err error
		cursorVal, err = query.DecodeCursor(cursor)
		if err != nil {
	...
	var err error
	if cursor == "" {
		rows, err = db.QueryContext(ctx, paginateUsersCursorSQL, limitN)
	} else {
		rows, err = db.QueryContext(ctx, paginateUsersCursorSQLAfter, cursorVal, limitN)
	}
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	out := make([]PaginateUsersCursorRow, 0, limitN)
	for rows.Next() {
		row, err := scanPaginateUsersCursorRow(rows)
		if err != nil {
			return nil, err
		}
		out = append(out, row)
	...
	if hasMore && len(out) > 0 {
		if val := query.CursorValue(out[len(out)-1], "id"); val != nil {
			next = query.EncodeCursor(val)
		}
	}
	return &query.PageResult[PaginateUsersCursorRow]{
		Data:       out,
		Style:      string(query.PageCursor),
		PerPage:    perPage,
		NextCursor: next,
		HasMore:    hasMore,
	}, nil
