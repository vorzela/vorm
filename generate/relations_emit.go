...
func renderTableRelations(t introspect.Table, tables []introspect.Table, plans []relPlan, fields map[string]map[string]string) string {
	if len(plans) == 0 {
		return ""
	}
	...
	for _, p := range plans {
		...
		if p.MorphTypeColumn != "" {
			fmt.Fprintf(&b, "\t\tMorphTypeColumn: %q,\n", p.MorphTypeColumn)
		}
		...
		case query.RelationHasManyThrough:
			...
			fmt.Fprintf(&b, "\t\t\tFarKey:       %q,\n", p.ForeignKey)
			fmt.Fprintf(&b, "\t\t\tParentKey:    func(m *%s) any { return m.%s },\n", owner, ownerField)
			fmt.Fprintf(&b, "\t\t\tAssign:       func(m *%s, rows []%s) { m.%s = rows },\n", owner, related, p.Field)
			b.WriteString("\t\t})\n")

		case query.RelationHasOneOfMany:
			relatedField, ok := relatedFields[p.ForeignKey]
			if !ok {
				b.WriteString("\t\treturn nil\n\t})\n")
				continue
		...
		case query.RelationMorphToMany:
			relatedField, ok := relatedFields[p.RelatedKey]
			if !ok {
				b.WriteString("\t\treturn nil\n\t})\n")
				continue
	...
	b.WriteString("}\n")
	b.WriteString(renderAssocMethods([]introspect.Table{t}, map[string][]relPlan{t.Name: plans}, fields))
	return b.String()
}
...
func renderAssocMethods(tables []introspect.Table, rels map[string][]relPlan, fields map[string]map[string]string) string {
	...
	for _, t := range tables {
		...
		for _, p := range rels[t.Name] {
			if p.Kind != query.RelationBelongsToMany && p.Kind != query.RelationMorphToMany {
				continue
			}
			...
			fmt.Fprintf(&b, "\t\tParentID: m.%s, CreatedAt: %t, UpdatedAt: %t, UniquePair: %t,\n", idField, p.PivotCreatedAt, p.PivotUpdatedAt, p.PivotUnique)
			if p.MorphType != "" {
				fmt.Fprintf(&b, "\t\tMorphType: %q, MorphTypeColumn: %q,\n", p.MorphType, p.MorphTypeColumn)
			}
			b.WriteString("\t}\n}\n")
